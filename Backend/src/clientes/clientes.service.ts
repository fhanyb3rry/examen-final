...
export class ClientesService {
  ...
  // Listar todos los clientes (sin filtro por activo)
  async findAll() {
  ...
  // Nuevo método: obtener todos los clientes (sin relaciones pesadas)
  ...
  private generateClientes(cantidad: number): Partial<Cliente>[] {
    ...
    for (let i = 1; i <= cantidad; i++) {
      ...
      clientes.push({
        ...
        // ⬇️ QUITADO: activo: Math.random() > 0.1,
      });
    }
  ...
  // ⬇ MÉTODO ELIMINADO o MODIFICADO - ya no filtramos por activo
  ...
  const clientesConTotales = await Promise.all(
    clientes.map(async (cliente) => {
      ...
      // ⬇️ NO uses spread operator, especifica cada campo
      return {
        // Solo los campos que quieres
        id_cliente: cliente.id_cliente,
        nombre: cliente.nombre,
        apellido: cliente.apellido,
        ...
        // NO incluyas 'activo' aunque exista
